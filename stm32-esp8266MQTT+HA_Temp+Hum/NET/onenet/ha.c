...
//	
//	body_len = HA_FillBuf(buf);																	//获取当前需要发送的数据流的总长度
//	
//	if(body_len)
//	{
		...
		case MQTT_PKT_PUBLISH:																//接收的Publish消息
			...
			if(result == 0)
			{
				...
				if(data_ptr)
				{
					...
					HA_Publish(topic_buf, "ojbk");										//回复命令
				}
			}
...
//==========================================================
//	函数名称：	HA_Device_registration
//
//	函数功能：	向HA注册设备与实体
//
//	入口参数：	Device_name: 注册设备名  Entity_name：注册实体名  Type_name：实体类型  Entity_ID：实体编号
//
//	返回参数：	homeassistant_state_topic：数据上传的话题
//
//	说明：		
//==========================================================
...
char *HA_Device_registration(struct Dev_info_Tpye dev_info)
{
    ...
    // 添加键值对到JSON对象，实体信息
    cJSON_AddStringToObject(root, "unique_id", unique_id);  //实体唯一ID
    cJSON_AddStringToObject(root, "name", Entity_name);     //实体名
    cJSON_AddStringToObject(root, "state_topic", state_topic);      //上传数据话题名
